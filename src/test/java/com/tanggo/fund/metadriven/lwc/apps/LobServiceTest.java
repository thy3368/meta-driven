...
class LobServiceTest {
    ...
    void testPlaceOrder() {
        ...
        assertTrue(result.getDate() instanceof PlaceOrderCommandHandler.PlaceOrderResult);

        PlaceOrderCommandHandler.PlaceOrderResult orderResult =
            (PlaceOrderCommandHandler.PlaceOrderResult) result.getDate();
    ...
    void testOrderMatching() {
        ...
        PlaceOrderCommandHandler.PlaceOrderResult sellOrderResult =
            (PlaceOrderCommandHandler.PlaceOrderResult) sellResult.getDate();
        ...
        PlaceOrderCommandHandler.PlaceOrderResult buyOrderResult =
            (PlaceOrderCommandHandler.PlaceOrderResult) buyResult.getDate();

        // 验证成交
        assertEquals(1, buyOrderResult.getTrades().size());
        OrderBookService.Trade trade = buyOrderResult.getTrades().get(0);
    ...
    void testPartialFill() {
        ...
        assertEquals(new BigDecimal("100.0"),
            buyOrderResult.getTrades().get(0).getQuantity());

        // 买单剩余50应该挂在订单薄上
        assertEquals(new BigDecimal("50.0"),
            buyOrderResult.getOrder().getRemainingQuantity());
    ...
    void testCancelOrder() {
        ...
        CancelOrderCommandHandler.CancelOrderResult cancelOrderResult =
            (CancelOrderCommandHandler.CancelOrderResult) cancelResult.getDate();
    ...
    void testQueryOrderBook() {
        ...
        for (int i = 1; i <= 5; i++) {
            PlaceOrderCommand cmd = new PlaceOrderCommand(
                ...
                symbol,
                OrderSide.BUY,
                new BigDecimal(100 - i),  // 99, 98, 97, 96, 95
                new BigDecimal("10.0")
            );
        ...
        for (int i = 1; i <= 5; i++) {
            PlaceOrderCommand cmd = new PlaceOrderCommand(
                ...
                symbol,
                OrderSide.SELL,
                new BigDecimal(101 + i),  // 102, 103, 104, 105, 106
                new BigDecimal("10.0")
            );
            commandService.handleCommand(cmd);
        }
        ...
        OrderBookService.OrderBookSnapshot snapshot =
            (OrderBookService.OrderBookSnapshot) queryResult.getDate();
    ...
    void testComplexMatching() {
        ...
        // 卖方：在不同价位挂单
        commandService.handleCommand(new PlaceOrderCommand(
            "SELL-A", symbol, OrderSide.SELL,
            new BigDecimal("0.10"), new BigDecimal("1000")
        ));
        commandService.handleCommand(new PlaceOrderCommand(
            "SELL-B", symbol, OrderSide.SELL,
            new BigDecimal("0.11"), new BigDecimal("2000")
        ));
        commandService.handleCommand(new PlaceOrderCommand(
            "SELL-C", symbol, OrderSide.SELL,
            new BigDecimal("0.12"), new BigDecimal("3000")
        ));

        // 买方：市价吃单（价格设置很高，能吃掉所有卖单）
        PlaceOrderCommand bigBuyOrder = new PlaceOrderCommand(
            "BUY-BIG",
            symbol,
            OrderSide.BUY,
            new BigDecimal("0.15"),  // 高于所有卖单价格
        ...
        // 验证成交数量
        BigDecimal totalFilled = orderResult.getTrades().stream()
            .map(OrderBookService.Trade::getQuantity)
            .reduce(BigDecimal.ZERO, BigDecimal::add);
        assertEquals(new BigDecimal("5000"), totalFilled);

        System.out.println("✅ 复杂撮合测试成功:");
        orderResult.getTrades().forEach(trade ->
    ...
    void testPriceTimePriority() {
        ...
        // 同一价格，时间优先
        commandService.handleCommand(new PlaceOrderCommand(
            "SELL-TIME-1", symbol, OrderSide.SELL,
            new BigDecimal("20.00"), new BigDecimal("10")
        ));
        commandService.handleCommand(new PlaceOrderCommand(
            "SELL-TIME-2", symbol, OrderSide.SELL,
            new BigDecimal("20.00"), new BigDecimal("10")
        ));

        // 买单匹配
        PlaceOrderCommand buyCommand = new PlaceOrderCommand(
            "BUY-TIME", symbol, OrderSide.BUY,
